...
function inputChange() {
	...
	p3x = 0;//readParam('p3x') / 180.0 * Math.PI;

	p1y = readParam('p1y') / 180.0 * Math.PI;
	p2y = readParam('p2y') / 180.0 * Math.PI;
	p3y = 0;//readParam('p3y') / 180.0 * Math.PI;

	td1 = readParam('td1') / 1440.0;
	td2 = readParam('td2') / 1440.0;
	td3 = 0;//readParam('td3');
...
function swing() {
	var eta = 2.0 * Math.PI * paperRotation * t;

	// setup each pendulum equation
	var x1 = A1x * Math.sin(2.0 * Math.PI * f1 * t + p1x) * Math.exp(-td1 * t);
	var y1 = A1y * Math.sin(2.0 * Math.PI * f1 * t + p1y) * Math.exp(-td1 * t);
	var x2 = A2x * Math.sin(2.0 * Math.PI * f2 * t + p2x) * Math.exp(-td2 * t);
	var y2 = A2y * Math.sin(2.0 * Math.PI * f2 * t + p2y) * Math.exp(-td2 * t);
	var x3 = A3x * Math.sin(2.0 * Math.PI * f3 * t + p3x) * Math.exp(-td3 * t);
	var y3 = A3y * Math.sin(2.0 * Math.PI * f3 * t + p3y) * Math.exp(-td3 * t);
	...
	var xb = x * Math.cos(eta) - y * Math.sin(eta);
	var yb = y * Math.cos(eta) + x * Math.sin(eta);
	x = xb; y = yb;
